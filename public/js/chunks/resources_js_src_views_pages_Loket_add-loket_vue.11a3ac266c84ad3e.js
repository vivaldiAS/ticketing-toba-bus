...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  mounted: function mounted() {
    ...
    axios__WEBPACK_IMPORTED_MODULE_2___default().get("/admin-loket/all", {
      headers: {
        Authorization: "Bearer ".concat(access_token)
      }
    }).then(function (response) {
      _this.adminloket = response.data.map(function (item) {
        return {
          name: item.name,
          id: item.id
        };
