  ...
  computed: {
    formattedSchedules: function formattedSchedules() {
      ...
      this.schedules.forEach(function (schedule) {
        ...
        if (!formatted.some(function (item) {
          return item[day];
        })) {
          var item = (0,E_DantobBuss_Application_node_modules_babel_runtime_helpers_esm_defineProperty__WEBPACK_IMPORTED_MODULE_0__["default"])({}, day, schedule.nomor_pintu);

          formatted.push(item);
        } else {
          formatted.forEach(function (item) {
            if (item[day]) item[day] = schedule.nomor_pintu;
          });
        }
      });
      return formatted;
