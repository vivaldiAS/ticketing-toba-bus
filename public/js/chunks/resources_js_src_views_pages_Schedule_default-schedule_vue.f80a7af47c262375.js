...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  computed: {
    formattedSchedules: function formattedSchedules() {
      ...
      var formatted = []; // Inisialisasi struktur data untuk setiap hari

      for (var i = 0; i < 7; i++) {
        formatted.push({});
      } // Mapping data schedules ke dalam formatted


      this.schedules.forEach(function (schedule) {
        var dayIndex = schedule.day_of_week;
        var day = _this.daysOfWeek[dayIndex];

        if (!formatted[dayIndex][day]) {
          formatted[dayIndex][day] = [];
        }

        formatted[dayIndex][day].push(schedule.nomor_pintu);
      }); // Urutkan setiap hari berdasarkan waktu

      formatted.forEach(function (day) {
  ...
  methods: {
    fetchSchedules: function fetchSchedules() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_4___default().get('/api/default-schedules', {
        headers: {
