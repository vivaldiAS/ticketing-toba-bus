...
/* harmony default export */ __webpack_exports__["default"] = ({
  setup: function setup() {
    ...
    var selectedBusType = (0,_vue_composition_api__WEBPACK_IMPORTED_MODULE_5__.ref)("");
    var selectedBusPrice = (0,_vue_composition_api__WEBPACK_IMPORTED_MODULE_5__.ref)("");
    return {
      schedule: schedule,
      bus: bus,
      route: route,
  ...
  mounted: function mounted() {
    ...
    axios__WEBPACK_IMPORTED_MODULE_4___default().get("/api/routes/show/all", {
      ...
      _this.route.value = response.data.data.filter(function (item) {
    ...
    axios__WEBPACK_IMPORTED_MODULE_4___default().get("/api/buss/show/all", {
      ...
      _this.bus.value = response.data.data.filter(function (item) {
        return item.status === 1;
      }).map(function (item) {
        return {
          id: item.id,
    ...
    axios__WEBPACK_IMPORTED_MODULE_4___default().get("/api/schedule/show/".concat(id), {
      ...
      _this.schedule.value = response.data.data[0];
      _this.schedule.value.route_id = response.data.data[0].route_id;
    })["catch"](function (error) {
      console.log(error);
    });
  },
  methods: {
    saveSelectBus: function saveSelectBus() {
      ...
      var selectedBus = this.bus.value.find(function (bus) {
        return bus.id === _this2.schedule.value.bus_id;
      });

      if (selectedBus) {
        this.selectedBusType.value = selectedBus.type;
        this.schedule.value.bus_id = selectedBus.id;
        this.schedule.value.harga = null;
      }
    },
    saveSelectRoute: function saveSelectRoute() {
      var _this3 = this;

      var selectedRoute = this.route.value.find(function (route) {
        return route.id === _this3.schedule.value.route_id;
      });

      if (selectedRoute) {
        this.selectedBusPrice.value = selectedRoute.harga;
        this.schedule.value.route_id = selectedRoute.id;
        this.schedule.value.harga = selectedRoute.harga;
      }
    },
    AddSchedule: function AddSchedule() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_4___default().post("api/schedule/add", {
        bus_id: this.schedule.value.bus_id,
        route_id: this.schedule.value.route_id,
        tanggal: this.schedule.value.tanggal,
        harga: this.schedule.value.harga
      }, {
        headers: {
          Authorization: "Bearer ".concat(access_token)
        }
      }).then(function (response) {
        console.log(_this4.schedule.value);
