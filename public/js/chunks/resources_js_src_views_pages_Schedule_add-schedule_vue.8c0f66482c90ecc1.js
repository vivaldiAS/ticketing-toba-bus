...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  mounted: function mounted() {
    ...
    axios__WEBPACK_IMPORTED_MODULE_6___default().get("/api/buss/show/noJadwal", {
      headers: {
        Authorization: "Bearer ".concat(access_token)
      }
    }).then(function (response) {
      ...
      .map(function (item) {
        return {
          ...
          type: item.type,
          lokasi_loket: item.lokasi_loket
        };
      });
  ...
  methods: {
    saveSelectBus: function saveSelectBus() {
      ...
      if (selectedBus) {
        ...
        // Ambil lokasi loket dari bus yang dipilih

        var selectedBusLocation = selectedBus.lokasi_loket; // Filter rute berdasarkan lokasi loket bus yang dipilih

        this.route = this.route.filter(function (route) {
          // Cek apakah derparture atau arrival rute mengandung lokasi loket bus
          return route.derpature.includes(selectedBusLocation) || route.arrival.includes(selectedBusLocation);
        });
      }
    },
    ...
    AddSchedule: function AddSchedule() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_6___default().post("/api/schedule/add", // Ensure correct URL with leading slash
      {
