...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  computed: {
    filteredRoutes: function filteredRoutes() {
      ...
      return this.route.filter(function (route) {
        return route.type === _this.selectedBusType;
      });
    }
  },
  mounted: function mounted() {
    ...
    axios__WEBPACK_IMPORTED_MODULE_5___default().get("/api/routes/show/all", {
      ...
      _this2.route = response.data.data.filter(function (item) {
    ...
    axios__WEBPACK_IMPORTED_MODULE_5___default().get("/api/buss/show/all", {
      ...
      _this2.bus = Object.values(response.data.data).filter(function (item) {
        return item.status == 1;
      }).map(function (item) {
        return {
          id: item.id,
    ...
    axios__WEBPACK_IMPORTED_MODULE_5___default().get("/api/schedule/show/".concat(id), {
      ...
      _this2.schedule.route_id = response.data.data[0].route_id;
    })["catch"](function (error) {
      console.log(error);
    });
  },
  methods: {
    saveSelectBus: function saveSelectBus() {
      ...
      var selectedBus = this.bus.find(function (bus) {
        return bus.id === _this3.schedule.bus_id;
      });
