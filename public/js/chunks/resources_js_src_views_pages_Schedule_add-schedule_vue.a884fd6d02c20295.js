...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    AddSchedule: function AddSchedule() {
      ...
      }); // Dapatkan lokasi locket dari bus yang dipilih

      var selectedBus = this.bus.find(function (bus) {
        return bus.id === _this4.schedule.bus_id;
      });
      var lokasiLoket = selectedBus.lokasi_loket; // Mengambil rute berdasarkan lokasi locket

      axios__WEBPACK_IMPORTED_MODULE_4___default().get("/api/routes/show/all", {
        params: {
          lokasi_loket: lokasiLoket
        },
        headers: {
          Authorization: "Bearer ".concat(access_token)
        }
      }).then(function (response) {
        _this4.route = response.data.data.filter(function (item) {
          return item.status == 1;
        }) // Filter dengan supir.status = 1
        .map(function (item) {
          return {
