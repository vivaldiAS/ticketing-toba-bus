<template>
  <div>
              ...
              <div class="row">
                ...
                <v-row class="col-md-6 d-flex">
                  ...
                  >
                    ...
                    <nav-menu-link :to="{ name: 'pages-admin-loket' }">
                      <v-btn
                        small
                        color="primary"
    ...
    <v-card v-if="pesanan.length === 0" style="margin-top: 10px;">
      <h3 class="text-center py-4">Maaf, anda belum memiliki pesanan.</h3>
    </v-card>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    navigateToDetailPage(item) {
      const routeName = item.method === "cash" ? "loket-payment" : "payment-mt";
      this.$router.push({ name: routeName, params: { bookings_id: item.bookings_id } });
    },
      ...
      axios
        ...
        .then((response) => {
          this.pesanan = response.data.filter(
            (pesanan) => pesanan.status === "in_progress" || pesanan.status === "not_started"
          );
