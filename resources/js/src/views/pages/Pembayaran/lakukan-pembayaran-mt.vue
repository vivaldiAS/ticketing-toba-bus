<template>
    <div>
        <v-card v-for="item in schedule" :key="item.id">
            <v-card-title>Lakukan Pembayaran dengan memilih Virtual Account</v-card-title>
            <div class="container mt-3 text-center">
                ...
                <h3>Silakan lakukan pembayaran ke Virtual Account yang dituju</h3>
                ...
                <div v-if="method && status && expiredDate">
                    ...
                    <v-btn @click="bayarSekarang" color="primary">Bayar Sekarang</v-btn>
                </div>
            </div>
        </v-card>
    </div>
        ...
        getPaymentData(paymentId) {
            ...
            axios
                ...
                .then((response) => {
                    ...
                    this.snapToken = paymentData.snap_token; // Simpan snap_token
    ...
    window.snap.pay(snapToken, {
        onSuccess: (result) => {
            ...
            if (this.status === "Berhasil") {
                this.$router.push({ path: '/pesananku' });
            } else {
                window.location.href = `tiket/${result.order_id}`;
            }
            console.log(result);
        },
        onPending: (result) => {
